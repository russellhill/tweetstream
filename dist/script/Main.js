...
function reset() {
	...
	// color theme
	theme = themes[0]; //themes[ Math.random() * themes.length >> 0 ];
...
function createItem(message_text, with_extras) {

	var size = 250;

	var element = document.createElement( 'div' );
	element.id = 'rotate_me';
	element.width = size;
	element.height = size;	
	...
	graphics.fillStyle = theme[ (Math.random() * 4 >> 0) + 1]; //theme[ 3 ];
	graphics.beginPath();
	...
	text.innerHTML = '<span style="color:' + theme[0] + ';">' + message_text + '</span>';
